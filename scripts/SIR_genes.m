...
classdef SIR_genes
    ...
    methods
        ...
        function self = SIR_genes(opt, clear_names)
            ...
            load(ws, 'gene_expr');
            % load(ws, 'gene_expr_rand');
            % gene_expr = gene_expr_rand;
        ...
        function risk_names = set_risk_names(self, opt, gene_expr)          %#ok<INUSD>
            ...
            if ~isfield(opt, 'risk_names')
                ...
                elseif opt.parc == "S332_PCA"           % 165 risk PCs
                    ...
                    % opt.risk_names = string(gene_expr.Properties.VariableNames(1:5));
                    % load('data/workspace_' + opt.parc + '.mat', 'risk_names');
                    % opt.risk_names = risk_names;
                end
            end
